...
import {StyleSheet,Button, View , Text, ActivityIndicator,ScrollView,TouchableOpacity, Image, Share, Platform} from 'react-native'
...
class FilmDetail extends React.Component{
  constructor(props){
    super(props)
    this.state={
      film: undefined,//Pour l'instanr on pas les details du film
      isLoading:true//A l'ouverture de la vue , on affiche le chargement, le temps de recuperer le detail du film
    }
  }
  componentDidMount(){
    getFilmsDetailFromApi(this.props.navigation.getParam('idFilm')).then(data=>{
        this.setState({
          film: data,
          isLoading:false
        })
  ...
  _displayFavoriteImage(){
    var sourceImage=require('../image/like1.png')
    var shouldEnlarge=false

    if(this.props.favoritesFilm.findIndex(item=>item.id===this.state.film.id) !==-1){
      //Film dans nos favoris
      sourceImage=require('../image/like.png');
      shouldEnlarge=true
    }
    return(
      ...
      return (
        <ScrollView style={styles.scrollview_container}>
            ...
            <Text style={styles.default_text}>Sorti le
                {moment(
                  new Date(film.release_date)).format('DD/MM/YYYY')
                }
            </Text>
  ...
  _displayLoading(){
    if(this.state.isLoading){
      //Si isLoading vaut true , on affiche le chargement a l'ecran
      return (
  ...
  _displayFloatingActionButton(){
    ...
    if(film!=undefined && Platform.OS=='android' ){
      //Uniquement sur android et lorsque le film est charger
      return (
        <TouchableOpacity
            style={styles.share_touche}
